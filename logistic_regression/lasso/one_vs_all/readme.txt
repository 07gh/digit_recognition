Logistic Regression with Lasso Penalty with one vs. all multiclassification

One vs. all achieves 89.74% accuracy.
classify_digits_lasso_one_vs_all.m runs the classification. This classification technique runs cross-validation on lambda.

Learn the model on logit_lasso_one_vs_all_learn.m. Automatically adds a column of ones
Inputs:
...
Inputs:
	X -- Data to be classified. Same format as X in logit_lasso_one_vs_all_learn.m (Do not include column of ones)
	beta_vector -- Output from logit_lasso_one_vs_all_learn.m
...
logit_lasso_one_vs_all_cross_val.m. Helper function for logit_lasso_one_vs_all_learn.m. Runs cross validation on lambda.
Inputs:
	X_train -- Same as in logit_lasso_one_vs_all_learn.m
	y_train -- Same as in logit_lasso_one_vs_all_learn.m
	X_val -- Same as in logit_lasso_one_vs_all_learn.m
	y_val -- Same as in logit_lasso_one_vs_all_learn.m
	start -- Same as in logit_lasso_one_vs_all_learn.m
