...
classify_logistic_regression_ridge.m runs the classification on 2 classes. For > 2 classes, use the one_vs_all classification.

NB: Do not include a column of ones in X. The functions add it in automatically.

Learn the model on logistic_regression_ridge.m. This includes running cross-validation on lambda.
Inputs:
	...
	lambda -- Ridge penalty. Higher values of lambda penalize larger values of beta more
...
Once you've learned the model, test new observations using classify_logistic_regression_ridge.m
Inputs:
	X -- Data to be classified. Same format as X in logistic_regression.m (Do not add column of ones)
	beta -- Output from logistic_regression_ridge.m
	classes -- Output from logistic_regression_ridge.m
...
calc_gradient_ridge.m is a helper function for logistic_regression_ridge.m
...
Inputs:
	...
	lambda -- Ridge penalty. Same as in logistic_regression_ridge.
